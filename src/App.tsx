        ...
        <div className="container">
          ...
          {experience.length > ITEMS_PER_PAGE && (
            <div className="text-center mt-8">
              <button onClick={() => setShowAllExperience(!showAllExperience)} className="px-6 py-3 glass rounded-xl hover:bg-white/10 transition-colors text-gray-300">
      ...
      <section id="projects" className="py-20 relative z-10">
        <div className="container">
          ...
          {projects.length > 3 && (
            <div className="flex flex-wrap justify-center gap-4 mb-8">
              <div className="flex items-center gap-2">
                ...
                <select value={projectStatusFilter} onChange={(e) => setProjectStatusFilter(e.target.value)} className="px-4 py-2 bg-gray-800/80 border border-gray-700 rounded-lg text-sm text-gray-300 focus:outline-none focus:border-indigo-500">
              ...
              <div className="flex items-center gap-2">
                ...
                <select value={projectFilter} onChange={(e) => setProjectFilter(e.target.value)} className="px-4 py-2 bg-gray-800/80 border border-gray-700 rounded-lg text-sm text-gray-300 focus:outline-none focus:border-indigo-500">
                  <option value="all">All Technologies</option>
                  {allTechnologies.map(tech => (<option key={tech} value={tech}>{tech}</option>))}
                </select>
              </div>
              {(projectFilter !== 'all' || projectStatusFilter !== 'all') && (
                <button onClick={() => { setProjectFilter('all'); setProjectStatusFilter('all') }} className="px-4 py-2 text-indigo-400 hover:text-indigo-300 text-sm">
                  Reset Filters
                </button>
              )}
            </div>
            ...
            {displayedProjects.map((project, index) => (
              <div key={index} className="glass rounded-2xl overflow-hidden hover:border-indigo-500/50 transition-all group hover:-translate-y-1">
                <div className={`h-32 bg-gradient-to-br ${index % 3 === 0 ? 'from-purple-500 to-pink-500' : index % 3 === 1 ? 'from-blue-500 to-cyan-500' : 'from-green-500 to-emerald-500'} flex items-center justify-center relative`}>
                  <span className={`absolute top-3 left-3 px-2 py-1 rounded-full text-xs font-medium ${project.status === 'Completed' ? 'bg-green-500' : 'bg-yellow-500'}`}>{project.status}</span>
          ...
          {filteredProjects.length > 6 && (
            <div className="text-center mt-8">
              <button onClick={() => setShowAllProjects(!showAllProjects)} className="px-6 py-3 glass rounded-xl hover:bg-white/10 transition-colors text-gray-300">
      ...
      <section id="education" className="py-20 relative z-10">
        <div className="container">
          ...
          {education.length > ITEMS_PER_PAGE && (
            <div className="text-center mt-8">
              <button onClick={() => setShowAllEducation(!showAllEducation)} className="px-6 py-3 glass rounded-xl hover:bg-white/10 transition-colors text-gray-300">
